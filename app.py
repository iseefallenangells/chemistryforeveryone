...
if analyze_clicked:
    ...
    with c1:
        st.markdown(f'<div class="red-metric"><div class="red-metric-label">MW</div><div class="red-metric-value">{data["MW"]}</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="red-metric"><div class="red-metric-label">LogP</div><div class="red-metric-value">{data["LogP"]}</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="red-metric"><div class="red-metric-label">TPSA</div><div class="red-metric-value">{data["TPSA"]}</div></div>', unsafe_allow_html=True)
    with c4:
        lipinski_value = "✅ PASS" if violations == 0 else f"⚠️ {violations}"
        st.markdown(f'<div class="red-metric"><div class="red-metric-label">Lipinski</div><div class="red-metric-value">{lipinski_value}</div></div>', unsafe_allow_html=True)
